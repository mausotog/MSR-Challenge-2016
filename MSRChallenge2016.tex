...
\todo{High level todo, probably for Mau since I think it's his bibtex: please
  ...
  citations.}
\todo{Please move the bibitems included explicitly in this document into sigproc.bib
   and use bibtex.}
...
September 2015 provided by \cite{robert}. In the following subsections, we first
...
We used the Boa platform \cite{robert} to be able to query the Github repository
...
al. paper~\cite{dongsun}. The following is the description of each considered
pattern. 
...
tools for automatic software repair \cite{dongsun}\cite{weimer}\cite{claire}
first find a faulty statement using several different fault localization
techniques \cite{zach}, and once they have that faulty statement, they apply
...
A similar study was performed by Dongsun Kim et al. \cite{dongsun} in which they
look for the most common ways in which programmers patch bugs in software. The
researchers developed a variation of the tool Genprog \cite{weimer}\cite{claire}
with several different templates resembling patters programmers use to patch
bugs.

A good complement for this paper is \cite{hao}, where they search additions and
deletions in a smaller data set. In this paper, we focus on the replacements,
which have been left out of the analysis of \cite{hao}.
